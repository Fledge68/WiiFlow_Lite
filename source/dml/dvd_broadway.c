...
#define IOCTL_DI_ClearCoverInterrupt	0x86
#define IOCTL_DI_GetCoverStatus		0x88
...
#define IOCTL_DI_DVDLowAudioStatus	0xE2
...
#define IOCTL_DI_DISCTYPE 			0xF7
//#define DI_SETWBFSMODE_IOS222		0xfe

#define IOCTL_DI_SNEEK_READ_GAMEINFO  0x30
#define IOCTL_DI_SNEEK_SELECT_GAME	  0x23
#define IOCTL_DI_SNEEK_GET_GAMECOUNT  0x24
...
s32 bwDVD_LowStopMotor(u8 stop1,u8 stop2)
{
	...
	bufferin[0] = IOCTL_DI_STOPMOTOR<<24;
	bufferin[1] = stop1<<24;
	bufferin[2] = stop2<<24;

	DCFlushRange(bufferin, 0x20);

	int ret = IOS_Ioctl( __dvd_fd, IOCTL_DI_STOPMOTOR, bufferin, 0x20, bufferout, 0x20);
...
s32 bwDVD_LowRequestError( void )
{
	...
	bufferin[0] = (IOCTL_DI_REQERROR << 24);

	DCFlushRange(bufferin, 0x20);
	
	int ret = IOS_Ioctl( __dvd_fd, IOCTL_DI_REQERROR, bufferin, 0x20, bufferout, 0x20);

	DCFlushRange(bufferout, 0x20);
...
s32 bwDVD_LowAudioStatus( void )
{
	...
	bufferin[0] = (IOCTL_DI_DVDLowAudioStatus << 24);

	DCFlushRange(bufferin, 0x20);
	
	int ret = IOS_Ioctl( __dvd_fd, IOCTL_DI_DVDLowAudioStatus, bufferin, 0x20, bufferout, 0x20);
...
s32 bwDVD_LowClearCoverInterrupt(dvdcallbacklow cb)
{
	...
	cmd = ctx->cmd;
	cmd->diReg[0] = (IOCTL_DI_ClearCoverInterrupt<<24);
	ret = IOS_IoctlAsync(__dvd_fd,IOCTL_DI_ClearCoverInterrupt,cmd->diReg,sizeof(struct dicommand),__di_regvalcache,0x20,__dvd_iostransactionCB,ctx);
...
s32 bwDVD_LowReportKey(dvdcallbacklow cb)
{
	...
	cmd = ctx->cmd;
	cmd->diReg[0] = (IOCTL_DI_REPORTKEY<<24);
	ret = IOS_IoctlAsync(__dvd_fd,IOCTL_DI_REPORTKEY,cmd->diReg,sizeof(struct dicommand),__di_regvalcache,0x20,__dvd_iostransactionCB,ctx);
...
s32 bwDVD_GetCoverStatus(u32 *status)
{
	...
	bufferin[0] = (IOCTL_DI_GetCoverStatus << 24);
	DCFlushRange(bufferin, 0x20);
...
s32 bwDVD_SetDiscType(bool dvd5)
{
	...
	bufferin[0] = IOCTL_DI_DISCTYPE << 24;
	...
	int Ret = IOS_Ioctl(__dvd_fd, IOCTL_DI_DISCTYPE, bufferin, 0x20, bufferout, 0x20);
...
s32 Sneek_DVDReadGameInfo( u32 Offset, u32 Length, void *Data )
{
	...
	bufferin[0] = Offset;
	bufferin[1] = Length;
	bufferin[2] = MEM_VIRTUAL_TO_PHYSICAL((u32)Data);

	DCFlushRange(bufferin, 0x20);
	DCInvalidateRange(Data, Length);

	s32 ret = IOS_Ioctl( __dvd_fd, IOCTL_DI_SNEEK_READ_GAMEINFO, bufferin, sizeof(u32) * 3, NULL, 0 );
...
s32 Sneek_DVDSelectGame( u32 SlotID )
{
	...
	s32 ret = IOS_Ioctl(__dvd_fd, IOCTL_DI_SNEEK_SELECT_GAME, bufferin, sizeof(u32) * 1, NULL, 0 );
...
{
	s32 ret = IOS_Ioctl( __dvd_fd, IOCTL_DI_SNEEK_GET_GAMECOUNT, NULL, 0, (u32 *)(MEM_VIRTUAL_TO_PHYSICAL(Count)), sizeof(u32) );
	return ret;
}
